...
# What is the element on index 2? it is 5 , because indexing starts
with 0
...
# print(f"friends[4]: {friends[4]}") # IndexError: list index out of
range
print(
 f"friends[-1]: {friends[-1]}") # look at the right side of your
list, negative indexes starting from the last element
print(f"friends[-3]: {friends[-3]}")
print(f"odds[-3]: {odds[-3]}")
# Adding elements:
# list.append(new_element) - this adds new_element to end of the list
# list.insert(index, new_element) - this adds new_element on the
indicated index, shifts all elements to right
...
# friends[7] = 'elon' # IndexError: list assignment index out of
range
# print(f"new friends list after adding new: {friends}")
# to comment do >> ctrl + /
# Remove the elements: by value, by index
friends.remove('mark')
# removed_one1 = friends.remove('mark') - this is not valid statement,
since remove() does not return anything
# print(removed_one1)
print(f"new friends list after removing 'mark': {friends}")
removed_friends = []
removed_one = friends.pop(4) # pop() function returns (informs) what
it is removing
print(removed_one)
...
friends = ['jackson', 'said', 'lenur', 'tyson']
print(f"Hi {friends[0].title()}, I would like to invite you to family
dinner tomorrow.")
print(f"Hi {friends[2].title()}, I would like to invite you to family
dinner tomorrow.")
print(f"Hi {friends[1].title()}, I would like to invite you to family
dinner tomorrow.")
print(f"Hi {friends[3].title()}, I would like to invite you to family
dinner tomorrow.")
# H/W: exercises 3-1, 3-2, 3-5, 3-6, 3-7
names = ['jane', 'nicole', 'sophia', 'rachel', 'maria']
removed_guests = []
removed_guests.append(names.pop())
print(removed_guests[-1].title() + ", I am sorry that I can’t invite
you to dinner.")
...
names.sort(reverse=True) # changing the list sorting in descending
order
print(names)
...
sorted_cars_desc = sorted(cars, reverse=True) # copy of the list
sorted in desc order
print(f"cars: {cars}")
print(f"sorted_cars_asc: {sorted_cars_asc}")
print(f"sorted_cars_desc: {sorted_cars_desc}")
cars.reverse() # it is just reverses the list, does not do any type
of sorting
print(f"cars: {cars}")
sorted_cars_asc2 = sorted(cars) # copy of the list sorted in asc
order
...
print(bucket_list)
print(f"sorted(bucket_list, reverse=True): {sorted(bucket_list,
reverse=True)}")
print(bucket_list)
...
print(bucket_list)
# None : null in sql
# object : everything is an object in the python (character, file,
string, variable, list, function etc. )
# iterable : something that can be iterate (loop) most data
structures, string, not a number, not boolean
# return statement: some functions return an object so we can use in
print to bring up to the STDOUT (console), but some functions do not
return anything (None) so the result of the print(object) will be None